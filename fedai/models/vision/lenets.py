"""Lenet-based CNN models"""

# AUTOGENERATED! DO NOT EDIT! File to edit: ../../../nbs/02b_models.vision.lenets.ipynb.

# %% auto #0
__all__ = ['MNISTCNNBackbone', 'MNISTCNN', 'CIFAR10CNNBackbone', 'CIFAR10CNN', 'FedAvgCNNBackbone', 'FedAvgCNN', 'make_lenet']

# %% ../../../nbs/02b_models.vision.lenets.ipynb #5ce10414
from fastcore.utils import * 
from torch import nn
import torch.nn.functional as F
import torch
from typing import Tuple, List, Dict, Any

# %% ../../../nbs/02b_models.vision.lenets.ipynb #5b64a343
class MNISTCNNBackbone(nn.Module):
    def __init__(self, 
                 img_size: Tuple[int, int, int]= (1, 28, 28), 
                 hidden_dim: int=512):
        
        super(MNISTCNNBackbone, self).__init__()
        in_channels, h, w = img_size    
        self.hidden_dim = hidden_dim

        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels, 32, kernel_size=5, stride=1, padding=2),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2)
        )

        self.conv2 = nn.Sequential(
            nn.Conv2d(32, 64, kernel_size=5, stride=1, padding=2),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2)
        )

        final_size = h // 2 // 2
        self.fc1 = nn.Linear(64 * final_size * final_size, hidden_dim)
        

    def forward(self, x):
        out = self.conv1(x)
        out = self.conv2(out)
        out = torch.flatten(out, 1)
        out = self.fc1(out)
        return out

    

# %% ../../../nbs/02b_models.vision.lenets.ipynb #54684cd7
class MNISTCNN(nn.Module):
    def __init__(self, 
                 img_size: Tuple[int, int, int]= (1, 28, 28), 
                 hidden_dim: int=512,
                 num_classes: int= 10):
        
        super(MNISTCNN, self).__init__()
        in_channels, h, w = img_size    
        self.hidden_dim = hidden_dim

        self.backbone = MNISTCNNBackbone(img_size, hidden_dim)
        self.head = nn.Linear(hidden_dim, num_classes)

    def forward(self, x):
        out = self.backbone(x)
        out = self.head(out)
        return out
    

# %% ../../../nbs/02b_models.vision.lenets.ipynb #2b56fcda
import torch
import torch.nn as nn
import numpy as np

class CIFAR10CNNBackbone(nn.Module):
    def __init__(self, 
                 img_size: Tuple[int, int, int]= (3, 32, 32), 
                 hidden_dim: int=512):
        
        super(CIFAR10CNNBackbone, self).__init__()
        in_channels, h, w = img_size
        self.hidden_dim = hidden_dim

        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels, 32, kernel_size=3, stride=1, padding=1),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2)
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2)
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2)
        )

        final_size = h // 2 // 2 // 2  # 3 pool layers divide the size by 2 three times
        self.fc1 = nn.Sequential(nn.Linear(128 * final_size * final_size, hidden_dim),
                                 nn.ReLU(inplace=True)
                                 )


    def forward(self, x):
        out = self.conv1(x)
        out = self.conv2(out)
        out = self.conv3(out)
        out = torch.flatten(out, 1)
        out = self.fc1(out)
        return out



# %% ../../../nbs/02b_models.vision.lenets.ipynb #307e6a21
class CIFAR10CNN(nn.Module):
    def __init__(self, 
                 img_size: Tuple[int, int, int]= (3, 32, 32), 
                 hidden_dim: int=512,
                 num_classes: int= 10):
        
        super(CIFAR10CNN, self).__init__()
        self.hidden_dim = hidden_dim
        self.backbone = CIFAR10CNNBackbone(img_size, hidden_dim)
        self.head = nn.Linear(hidden_dim, num_classes)
        

    def forward(self, x):
        out = self.backbone(x)
        out = self.head(out)
        return out

# %% ../../../nbs/02b_models.vision.lenets.ipynb #a212e88f
class FedAvgCNNBackbone(nn.Module):
    def __init__(self, 
                 img_size: Tuple[int, int, int]= (3, 32, 32), 
                 hidden_dim: int=512):
        super().__init__()

        in_channels, h, w = img_size
        self.hidden_dim = hidden_dim
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels,
                      32,
                      kernel_size=5,
                      padding=0,
                      stride=1,
                      bias=True),
            nn.ReLU(inplace=True), 
            nn.MaxPool2d(kernel_size=(2, 2))
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(32,
                      64,
                      kernel_size=5,
                      padding=0,
                      stride=1,
                      bias=True),
            nn.ReLU(inplace=True), 
            nn.MaxPool2d(kernel_size=(2, 2))
        )

        def compute_dim(h, w):
            h = (h - 4) // 2  # After conv1 and pool
            w = (w - 4) // 2
            h = (h - 4) // 2  # After conv2 and pool
            w = (w - 4) // 2
            return 64 * h * w


        dim = compute_dim(h, w)
        self.fc1 = nn.Sequential(
            nn.Linear(dim, hidden_dim), 
            nn.ReLU(inplace=True)
        )

    def forward(self, x):
        out = self.conv1(x)
        out = self.conv2(out)
        out = torch.flatten(out, 1)
        out = self.fc1(out)
        return out

# %% ../../../nbs/02b_models.vision.lenets.ipynb #aa99f8e8
class FedAvgCNN(nn.Module):
    def __init__(self, 
                 img_size: Tuple[int, int, int]= (3, 32, 32), 
                 hidden_dim: int=512,
                 num_classes: int= 10):
        super().__init__()

        self.hidden_dim = hidden_dim
        self.backbone = FedAvgCNNBackbone(img_size= img_size, hidden_dim= hidden_dim)
        self.head = nn.Linear(hidden_dim, num_classes)

    def forward(self, x):
        x = self.backbone(x)
        x = self.head(x)
        return x

# %% ../../../nbs/02b_models.vision.lenets.ipynb #b03586a5
from .registery import register_model
from typing import Literal

# %% ../../../nbs/02b_models.vision.lenets.ipynb #038bb80c
@register_model(
    name='lenet',
    variants=['mnist', 'cifar10', 'fedavg']
)
def make_lenet(
    variant: Literal['mnist', 'cifar10', 'fedavg'] = 'fedavg',
    pretrained: bool = False, # Note: pretrained is not implemented for these simple models, but we include it in the signature for API consistency
    num_classes: int = 10,
    img_size: Tuple[int, int, int] = (3, 32, 32),
    hidden_dim: int = 512,
) -> nn.Module:
    
    model_dict = {
        'fedavg': FedAvgCNN,
        'mnist': MNISTCNN,
        'cifar10': CIFAR10CNN
    }

    if variant not in model_dict:
        raise ValueError(f"Variant '{variant}' not recognized. Available variants: {list(model_dict.keys())}")
    
    model_cls = model_dict[variant]
    model = model_cls(img_size=img_size, hidden_dim=hidden_dim, num_classes=num_classes)
    return model

