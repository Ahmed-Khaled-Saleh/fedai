...
algos=("feddbe")
kl_weight=(0 1 10 50)
momentum=(0 0.1 1)  # Example percentages for randomization

# 2. Build the combinations list
combinations=()
for a in "${algos[@]}"; do
    for k in "${kl_weight[@]}"; do
...
read -r ALGO K_IDX M_IDX <<< "$CURRENT_COMBO"

echo "------------------------------------------------"
echo "Job ID: $SLURM_ARRAY_JOB_ID | Task ID: $SLURM_ARRAY_TASK_ID"
echo "Running: $ALGO | KL Weight: $K_IDX | Momentum: $M_IDX"
echo "------------------------------------------------"
...
python main.py \
    ...
    server=puhti \
    optimizer=sgd \
    algorithm.kl_weight=$K_IDX \
    algorithm.momentum=$M_IDX