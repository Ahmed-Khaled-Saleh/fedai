...
#SBATCH --job-name=fedai_graph_types
#SBATCH --output=logs/fedai_%A_%a.out
#SBATCH --error=logs/fedai_%A_%a.err
#SBATCH --partition=gpu
#SBATCH --array=0-23          # Number of algorithms (0 to N-1)
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
for a in "${algos[@]}"; do
    for d in "${datasets[@]}"; do
        for nc in "${num_clients[@]}"; do
            for bgf in "${build_graph_freq[@]}"; do
                combinations+=("$a|$d|$nc|$bgf")
            done
        done
    done
done
...
CURRENT_BUILD_GRAPH_FREQ=$(echo $current_pair | cut -d'|' -f4)
...
python main.py \
    ...
    algorithm.build_graph_freq=$CURRENT_BUILD_GRAPH_FREQ